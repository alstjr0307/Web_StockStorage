...
class PostPopularLV(ListView,FormView):
    ...
    template_name = 'blog/post_popular_all.html'
    context_object_name = 'posts'
    paginate_by=20
    def get_queryset(self):
        return Post.objects.filter(category='F').annotate(num_likes=Count('likes')).filter(num_likes__gte=10)

class PostPopularDV(HitCountDetailView, FormView, MultipleObjectMixin,FormMixin):

    template_name='blog/post_popular_detail.html'
    paginate_by = 20
    count_hit = True
    form_class=NewCommentForm
    ...
    def get_success_url(self):	# post처리가 성공한뒤 행할 행동
        return reverse('blog:post_popular_detail', kwargs={'pk': self.object.pk})
    def get_queryset(self):
        return Post.objects.filter(category='F').annotate(num_likes=Count('likes')).filter(num_likes__gte=10)
    def get_context_data(self, **kwargs):
        self.object_list = self.get_queryset()
        ...
        context['comments'] = comments_connected
        context['user'] = self.request.user
        context['posts'] = Post.objects.filter(category='F').annotate(num_likes=Count('likes')).filter(num_likes__gte=10)
        if self.request.user.is_authenticated:
            context['comment_form'] = '1'
...
def PostPopularLike(request, pk):
    ...
    return HttpResponseRedirect(reverse('blog:post_popular_detail', args=[str(pk)]))
